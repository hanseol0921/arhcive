...
function ArchiveImport() {
  ...
  function isPdf(file) {
    return (
      file.type === "application/pdf" || getBaseName(file).endsWith(".pdf")
    );
  }
  ...
  function isVideoThumbnail(file) {
    return /(?:video.*_thumb|_thumb)\.(jpg|jpeg|png|webp)$/i.test(file.name);
  }
  ...
  function parsePdfMetadata(text, fallbackDate) {
    ...
    const dateTimeMatch = normalizedText.match(
      /(\d{4}-\d{2}-\d{2})[\s\n]+(\d{1,2}:\d{2})/,
    );

    const date = dateTimeMatch?.[1] || fallbackDate || "";

    const time = dateTimeMatch?.[2] ? dateTimeMatch[2].padStart(5, "0") : "";
    ...
    if (linkMatch) {
      ...
      let afterLink = normalizedText.slice(linkEndIndex).trim();

      // 첫 미디어 표시 위치
      const mediaMatch = afterLink.match(/(?:^|\n|\s)(?:사진|영상)\s*\d+/i);
  ...
  function extractVideoNames(pdfText) {
    const matches = [
      ...pdfText.matchAll(/([^\s()]+?\.(?:mp4|mov|webm|m4v))/gi),
    ];
  ...
  async function collectPostFolders(directoryHandle, parentPath = "") {
    const groups = [];
    ...
    for await (const [name, handle] of directoryHandle.entries()) {
      ...
      // 상위 분류 폴더만 재귀 탐색
      const childGroups = await collectPostFolders(handle, folderPath);
  ...
  async function createDraft(group) {
    const { folderPath, folderName, directoryHandle } = group;

    // 이 게시글을 실제로 초안으로 만들 때만
    // 해당 폴더의 파일을 읽는다.
    const files =
      group.files || (await readFilesFromDirectoryHandle(directoryHandle));

    const fallbackDate = getDateFromFolderName(folderName);
    ...
    const isMembership =
      folderPath.toLowerCase().includes("멤버쉽") ||
      folderPath.toLowerCase().includes("멤버십");
    ...
    const imageFiles = files
      .filter((file) => isImage(file) && !isVideoThumbnail(file))
    ...
    // PDF에서 발견한 영상 파일명
    const pdfVideoNames = extractVideoNames(pdfText);
    ...
    const videoItems = videoFiles.map((file) => {
      ...
      const thumbnailFile = files.find(
        (candidate) =>
          ...
          candidate.name
            .toLowerCase()
            .includes(base.replace(/_\d+$/, "").toLowerCase()) &&
          /_thumb\./i.test(candidate.name),
      );
      ...
      return {
        ...
        thumbnailPreviewUrl: thumbnailFile ? makePreviewUrl(thumbnailFile) : "",
    ...
    const sortedVideos = [...videoItems].sort((a, b) => {
      const aIndex = pdfVideoNames.findIndex(
        (name) => name.toLowerCase() === a.fileName.toLowerCase(),
      );

      const bIndex = pdfVideoNames.findIndex(
        (name) => name.toLowerCase() === b.fileName.toLowerCase(),
      );
    ...
    const media = [...photoItems, ...sortedVideos];
    ...
    return {
      ...
      pdfFound: Boolean(pdfFile),

      postDate: metadata.date,

      postTime: metadata.time,
  ...
  async function chooseArchiveRoot() {
    ...
    try {
      ...
      const groups = await collectPostFolders(rootHandle, rootHandle.name);

      groups.sort((a, b) => naturalCompare(b.folderPath, a.folderPath));
      ...
      if (groups.length === 0) {
        alert("날짜 이름의 게시글 폴더를 찾지 못했습니다.");
      }
    } catch (error) {
      // 사용자가 폴더 선택 창을 취소한 경우는 오류 표시 X
      if (error?.name !== "AbortError") {
        console.error("백업 폴더 탐색 오류:", error);
        alert(`폴더를 읽는 중 오류가 발생했습니다.\n${error.message}`);
      }
    } finally {
      setParsing(false);
    }
  ...
  function toggleImportFolder(folderPath) {
  ...
  function selectAllImportFolders() {
    ...
    const pagePaths = foundGroups
      .slice(start, start + IMPORT_PAGE_SIZE)
      .map((group) => group.folderPath);

    setSelectedFolderPaths(pagePaths);
  }
  ...
  async function loadSelectedFolders() {
    ...
    try {
      const selectedGroups = foundGroups.filter((group) =>
        selectedFolderPaths.includes(group.folderPath),
      );
      ...
      const { data: existingPostRows, error: existingPostError } =
        await supabase
          .from("weverse_posts")
          .select("weverse_url")
      ...
      const existingPostUrls = new Set(
        (existingPostRows || [])
          .map((row) => String(row.weverse_url || "").trim())
          .filter(Boolean),
      );

      for (const group of selectedGroups) {
        const draft = await createDraft(group);

        const draftUrl = String(draft.postWeverseUrl || "").trim();

        if (draftUrl && existingPostUrls.has(draftUrl)) {
      ...
      // 날짜 → 시간 → 폴더명
      newDrafts.sort((a, b) => {
        const aKey = `${a.postDate} ${a.postTime}`;

        const bKey = `${b.postDate} ${b.postTime}`;

        const dateCompare = bKey.localeCompare(aKey);
      ...
      // 기존 초안을 날리지 않고 추가
      setDrafts((prev) => {
        const existingPaths = new Set(prev.map((draft) => draft.folderPath));

        const onlyNew = newDrafts.filter(
          (draft) => !existingPaths.has(draft.folderPath),
        );
      ...
      setFoundGroups((prev) =>
        prev.filter((group) => !selectedFolderPaths.includes(group.folderPath)),
      );
      setSelectedFolderPaths([]);
      ...
      console.error("선택 폴더 분석 오류:", error);
  ...
  function updateMedia(draftId, mediaId, field, value) {
    setDrafts((prev) =>
      prev.map((draft) => {
        if (draft.id !== draftId) {
          return draft;
  ...
  function safeFileName(fileName) {
    ...
    return `${Date.now()}_` + `${makeId()}` + extension;
  }
  ...
  async function uploadDraft(draftId) {
    const draft = drafts.find((item) => item.id === draftId);
    ...
    try {
      ...
      if (draft.postWeverseUrl) {
        const { data: existingPost, error: duplicateError } = await supabase
          .from("weverse_posts")
          .select("id")
          .eq("weverse_url", draft.postWeverseUrl)
          .maybeSingle();
      ...
      const { data: postData, error: postError } = await supabase
        ...
        .insert({
          ...
          content: draft.postContent.trim() || null,
      ...
      let nextUploadOrder = Number(latestPhoto?.upload_order || 0) + 1;
  ...
  const totalImportPages = Math.max(
    1,
    Math.ceil(foundGroups.length / IMPORT_PAGE_SIZE),
  );

  const importPageStart = (importPage - 1) * IMPORT_PAGE_SIZE;

  const visibleImportGroups = foundGroups.slice(
    importPageStart,
    importPageStart + IMPORT_PAGE_SIZE,
  );
